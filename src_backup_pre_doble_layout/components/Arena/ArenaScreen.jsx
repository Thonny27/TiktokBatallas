...
export default function ArenaScreen({ 
  ...
  return (
    <div className="relative flex flex-col h-full bg-black overflow-hidden select-none" onClick={onScreenTap}>
      
      {/* Header: Profiles and Viewers */}
      <div className="absolute top-0 w-full z-40 flex justify-between items-start p-4 pointer-events-none pt-6">
      ...
      <div className="absolute inset-0 z-20 cursor-pointer" onClick={onScreenTap}></div>

      {/* =========================================
          CONTENEDOR DINÁMICO DE VIDEO
          ========================================= */}
      <div 
        className={`w-full relative transition-all duration-700 ease-in-out z-10 ${
          isBattleActive ? 'h-[45%]' : 'absolute inset-0 h-full'
        }`}
      >
        {isBattleActive ? (
          /* MODO PK: PANTALLA DIVIDIDA */
          <div className="flex w-full h-full pointer-events-none relative">
            ...
            {/* PK Bar Anclada al borde inferior en Modo PK */}
            <div className="absolute bottom-0 left-0 w-full translate-y-1/2 z-40">
               <PKBar score1={score1} score2={score2} timeLeft={timeLeft} />
            </div>
          </div>
        ) : (
          /* MODO SOLO: PANTALLA COMPLETA */
          <div className="w-full h-full bg-gray-900 flex flex-col items-center justify-center relative pointer-events-none">
            ...
            {/* Gradiente inferior para proteger el chat en Modo Solo */}
      ...
      {/* =========================================
          ZONA INFERIOR (CHAT Y BOTONES)
          ========================================= */}
      <div 
        className={`w-full flex flex-col justify-end pointer-events-none z-30 transition-all duration-700 ${
          isBattleActive ? 'flex-1 bg-black pb-4' : 'absolute bottom-0 h-1/2 pb-4'
        }`}
      >
        ...
        {/* Chat Component */}
        <div className={`w-full px-4 mb-4 ${isBattleActive ? 'mt-8' : ''}`}>
           <AnimatePresence>
        ...
        <div className="w-full px-3 flex items-center justify-between pointer-events-auto space-x-2">
          ...
          {/* Iconos de Acción Derecha (Clon exacto de Tiktok) */}
          <div className="flex items-center space-x-2.5">
            ...
            <button 
              ...
              className="w-9 h-9 rounded-full bg-[#ff2a5f] flex items-center justify-center text-white shadow-[0_0_10px_rgba(255,42,95,0.4)] hover:scale-105 transition-all"
            >
