...
import { getCurrentTierIndex } from '../../utils/tiers';
...
export default function GiftCatalogModal({ isOpen, onClose, onSendGift, onOpenYape, balance, totalGiftsSent = 0 }) {
  ...
  const handleGiftClick = (gift) => {
    if (gift.id === 'caballero' && !isCaballeroUnlocked) {
      alert("Debes alcanzar el nivel Trono de Leyenda para usar este regalo exclusivo.");
      return;
    }
                ...
                gift.type === 'premium' ? (
                  <button
                    ...
                    className={`flex flex-col items-center justify-center cursor-pointer p-2 rounded-xl border col-span-2 active:scale-[0.98] transition group relative ${
                      gift.isEpic 
                        ? (!isCaballeroUnlocked && gift.id === 'caballero' ? 'grayscale opacity-50 bg-gray-800 border-gray-600' : 'bg-gradient-to-b from-yellow-900/50 to-transparent border-yellow-500/50 shadow-[0_0_20px_rgba(250,204,21,0.4)] hover:shadow-[0_0_25px_rgba(250,204,21,0.6)]') 
                  ...
                  <button
                    key={gift.id}
                    onClick={() => handleGiftClick(gift)}
                    className="flex flex-col items-center justify-center cursor-pointer hover:bg-gray-800 p-2 rounded-xl transition active:scale-95 group relative"
                  >
