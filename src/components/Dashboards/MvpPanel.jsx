...
export default function MvpPanel({ onLogout, onOpenYape, totalGiftsSent = 0, balance, setBalance }) {
  ...
  const maxExp = 5000;
  const progressPct = (currentExp / maxExp) * 100;
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-5 shadow-2xl mt-4">
          <div className="flex justify-between items-start mb-4">
            <div className="flex items-center space-x-4">
              <div className="w-16 h-16 rounded-2xl bg-gradient-to-tr from-cyan-500 to-pink-500 p-0.5 shadow-lg">
              ...
              <div>
                ...
                <div className="flex items-center space-x-1 mt-1 bg-black/40 rounded-full px-2 py-0.5 w-max">
                  <Trophy size={12} className="text-yellow-400" />
                  <span className="text-xs font-bold text-yellow-100">Nivel {currentLevel}</span>
                </div>
              </div>
            </div>
          ...
          <div className="mb-6">
            <div className="flex justify-between text-xs font-bold text-gray-400 mb-1.5">
              <span>Progreso a Nivel {currentLevel + 1}</span>
              <span className="text-cyan-400">{currentExp} / {maxExp}</span>
            </div>
            <div className="w-full h-2.5 bg-black/50 rounded-full overflow-hidden border border-white/5">
              <motion.div 
                initial={{ width: 0 }}
                animate={{ width: `${progressPct}%` }}
                className="h-full bg-gradient-to-r from-cyan-500 to-pink-500 rounded-full relative shadow-[0_0_10px_rgba(236,72,153,0.5)]"
              >
